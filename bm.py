...
import random
APP_ID =26148505
APP_HASH='8225ecac2c4b4a19dc6737b7e76377a9'
#bot_token='5829726533:AAHnVjD4h4oma1o8C1VHh9liImYAthTYdFI'
...
async def join_channel(event):
	try :
		await abbas(JoinChannelRequest('@TEMBM'))
...
async def OwnerStart(event):
    ...
    if sender.id == onerabbas_id:
        ...
        joinu = await abbas(JoinChannelRequest('TEMBM'))
        ...
        for i in range(100):
            ...
            if msgs.message.find('لا يوجد قنوات في الوقت الحالي , قم يتجميع النقاط بطريقه مختلفه') != -1:
                await shahm1.send_message(event.chat_id, f"تم الانتهاء من التجميع | bm")
                break
        ...
        await abbas.send_message(event.chat_id, "تم الانتهاء من التجميع | bm")
...
async def OwnerStart(event):
    ...
    if sender.id == onerabbas_id:
        ...
        for i in range(100):
            ...
            msgs = list.messages[0]
            if msgs.message.find('لا يوجد قنوات في الوقت الحالي , قم يتجميع النقاط بطريقه مختلفه') != -1:
                await abbas.send_message(event.chat_id, f"تم الانتهاء من التجميع | bm")
                break
            url = msgs.reply_markup.rows[0].buttons[0].url
            try:
                try:
    ...
    while True:
        try:
           ...
           if sender.id == onerabbas_id:
               ...
               joinu = await abbas(JoinChannelRequest('TEMBM'))
               channel_entity = await abbas.get_entity(pot)
               await abbas.send_message(pot, '**جاري بدأ عملية التجميع**')
               await abbas.send_message(pot, '/start')
...
async def OwnerStart(event):
    ...
    if sender.id == onerabbas_id:
        ...
        joion = await abbas(JoinChannelRequest('TEMBM'))
